...
public class XListView extends ListView implements OnScrollListener {
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (getFirstVisiblePosition() == 0
				...
				invokeOnScrolling();
					...
					&& (mFooterView.getBottomMargin() > 0 || deltaY < 0)&&!mFooterView.isTheLast) {
				// 最后一页，已停止或者想拉起
				Log.i("last", (-deltaY / OFFSET_RADIO) + "");
		...
		default:
			...
			if (getLastVisiblePosition() == mTotalItemCount - 1) {
				// 调用加载更多.
				if (mEnablePullLoad
						&& mFooterView.getBottomMargin() > PULL_LOAD_MORE_DELTA&&!mFooterView.isTheLast) {
	...
	/**
	 * 
	 * @param mark 1最后一条数据,2加载更多
	 */
	public void theLastItem(int mark){
		this.mark=mark;
		mFooterView.theLastItem(mark);
		if(mark==1){
		mFooterView.setOnClickListener(null);
		}else{
			mFooterView.setOnClickListener(new OnClickListener() {
